      ...
      await asyncForEach(
        ...
        async (preRequisiteCourse: IPrerequisiteCourseRequest) => {
          const createPrerequisite =
            await transactionClient.courseToPrerequsite.create({
              data: {
                courseId: result.id,
                preRequsiteId: preRequisiteCourse.courseId,
              },
            });
    ...
    if (preRequisiteCourses && preRequisiteCourses.length > 0) {
      ...
      await asyncForEach(
        ...
        async (deletePreCourse: IPrerequisiteCourseRequest) => {
          await transactionClient.courseToPrerequsite.deleteMany({
            where: {
              AND: [
                {
                  courseId: id,
                ...
                {
                  preRequsiteId: deletePreCourse.courseId,
                },
              ],
            },
          });
      ...
      await asyncForEach(
        ...
        async (insertPrerequisite: IPrerequisiteCourseRequest) => {
          await transactionClient.courseToPrerequsite.create({
            data: {
              courseId: id,
              preRequsiteId: insertPrerequisite.courseId,
            },
          });
        }
      );
...
const deleteByIdFromDB = async (id: string): Promise<Course> => {
  await prisma.courseToPrerequsite.deleteMany({
    where: {
      OR: [
        {
          courseId: id,
