...
import { StudentSemesterRegistrationCourseServices } from '../studentSemesterRegistationCourse/studentSemesterRegistationCourse.services';
import {
...
const startMyRegistration = async (
  ...
  if (!studentInfo) {
    throw new ApiError(httpStatus.FORBIDDEN, 'Student info not found');
  }
...
const confirmMyRegistration = async (
  ...
  if (!studentSemesterRegistration) {
    throw new ApiError(
      httpStatus.BAD_REQUEST,
      'Your are not recognized for this semester'
    );
  }
...
const getMySemesterRegistrationCourses = async (authUserId: string) => {
  ...
  if (!semesterRegistration) {
    throw new ApiError(
      httpStatus.BAD_REQUEST,
      'No semester register not found'
    );
  }
  ...
  const studentCurrentSemesterTaken =
